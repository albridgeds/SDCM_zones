function plotm_power_line (time,satelite,channel,receiver,mode)
...
%         end
        ...
        % если спутник за пределами рабочей зоне, переходим к следующему
        ...
        % вывод уровня мощности сигналов (с сохранением контура для последующего отображения)
        [satzone{s},~] = contourm(lat,long,zone_p,[receiver.Pmin receiver.Pmin],'LineColor','b','LineWidth',2);
        ...
        plot_map(worldmap);
        filename = sprintf('pictures\\powerl_%s',sat.name);
